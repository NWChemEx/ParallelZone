...
//--------------------------------------------------------------------
  ...
  Hardware::Hardware() {
	...
	HardwareComponent* hwcomp = HardwareComponent(dev_type_info[dev_type_id]);
...
int main() {
  ...
  if( nvmlDeviceGetCount( &true_device_count ) != NVML_SUCCESS ) {
    if( !world_rank  ) std::cout << "NVML Failed to Get Device Count" << std::endl;
    cleanup();
    return 1;
  }
  int cuda_device_count; cudaGetDeviceCount( &cuda_device_count );

  if(!world_rank) {
    std::stringstream ss;
    ss << "RANK = " << world_rank << " NGPU = " << true_device_count << " , CUDA_NGPU = " << cuda_device_count << std::endl;
    std::cout << ss.str();
  }
  ...
  if( std::any_of( cuda_device_counts_mpi.begin(), cuda_device_counts_mpi.end(),
    [&](auto c){ return c != cuda_device_count; } ) ) {

    if( !world_rank  ) std::cout << "This Driver Only Works If Every Rank has the same # devices" << std::endl;
    cleanup();
    return 1;
  ...
  for( int32_t i = 0; i < cuda_device_count; ++i ) {
    ...
    char pci_bus_str[20]; pci_bus_str[0] = '\0';
    ...
    uuid_str[0] = '\0';
    if( nvmlDeviceGetUUID( device, uuid_str, NVML_DEVICE_UUID_V2_BUFFER_SIZE ) ) {
      if(!world_rank) std::cout << "NVML Failed To Get UUID for IDX = " << i << std::endl;
      cleanup();
      return 1;
    }
    ...
    // Compute UUID HASH
    uuid_hashes[i] = 
      std::hash<std::string>{}(std::string(uuid_str));

    //std::stringstream msg;
    //msg << "RANK = " << world_rank << " DEVICE IDX = " << i << " UUID = " << uuid_str << " UUID HASH = " << uuid_hashes[i] << "\n";
    //std::cout << msg.str() << std::flush;
  }

  // Gather all UUIDs
  std::vector<std::size_t> gathered_uuid_hashes( world_size * cuda_device_count );
#ifdef ENABLE_MPI
  // Gather UUID Hashes
  MPI_Allgather( uuid_hashes.data(), cuda_device_count*sizeof(std::size_t), MPI_BYTE,
    gathered_uuid_hashes.data(), cuda_device_count*sizeof(std::size_t), MPI_BYTE,
    MPI_COMM_WORLD );
#else
  std::copy( uuid_hashes.begin(), uuid_hashes.end(), gathered_uuid_hashes.begin() );
#endif
  ...
  if( !world_rank ) {
    std::cout << "Found " << uniq_uuid.size() << " Unique Devices" << std::endl;
    for( auto uuid : uniq_uuid ) {
      ...
      while( cur_it != gathered_uuid_hashes.end() ) {
        cur_it = std::find( cur_it, gathered_uuid_hashes.end(), uuid );
        if( cur_it != gathered_uuid_hashes.end() ) {
          int idx = std::distance( gathered_uuid_hashes.begin(), cur_it );
          int mpi_rank = idx / cuda_device_count;
          int dev_idx  = idx % cuda_device_count;
          std::cout << "  RANK = " << mpi_rank << ", CUDA_INDEX = " << dev_idx << std::endl;
          cur_it++;
        }
      }
    }

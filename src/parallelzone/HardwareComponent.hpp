...
HardwareComponent::HardwareComponent(
  const PAPI_dev_type_info_t& papi_dev_type) {
...
HardwareComponent::initialize(const PAPI_dev_type_info_t& dev_type_info) {
    if(dev_type_info.id == PAPI_DEV_TYPE_ID__CPU && this->numDevices_ > 0) {
        PAPI_cpu_info_t* cpu_info =
          (PAPI_cpu_info_t*)dev_type_info.dev_info_arr;
        this->is_gpu_ = false;
        ...
        devInfo.major                     = -1;
        devInfo.minor                     = -1;
    } else if(dev_type_info.id == PAPI_DEV_TYPE_ID__NVIDIA_GPU &&
              this->numDevices_ > 0) {
        PAPI_gpu_info_u* gpu_info =
          (PAPI_gpu_info_u*)dev_type_info.dev_info_arr;
        this->is_gpu_ = true;

        devInfo.name      = gpu_info->nvidia.name;
        devInfo.warp_size = gpu_info->nvidia.warp_size;
        devInfo.max_threads_per_block =
          gpu_info->gpu_info->nvidia.max_threads_per_block;
        devInfo.max_blocks_per_multi_proc =
          gpu_info->nvidia.max_blocks_per_multi_proc;
        devInfo.max_shmem_per_multi_proc =
          gpu_info->nvidia.max_shmmem_per_block;
    ...
    } else if(dev_type_info.id == PAPI_DEV_TYPE_ID__AMD_GPU &&
              this->numDevices_ > 0) {
        PAPI_gpu_info_u* gpu_info =
          (PAPI_gpu_info_u*)dev_type_info.dev_info_arr;
        this->is_gpu_ = true;
        ...
        devInfo.max_threads_per_block =
          gpu_info->gpu_info->amd.max_threads_per_block;
        devInfo.max_blocks_per_multi_proc =
          gpu_info->amd.max_blocks_per_multi_proc;

...
Parallel::Parallel(MPI_Comm comm_world0) {
    ...
    npi[1] = npi[0];
    npi[2] = 1;
    for(int i = 0; i < npi[0]; ++i) procNd[i] = i;
}
...
double Parallel::MaxAll(const int d, const double sum) {
    ...
    if(npi[d] > 1)
        ...
        MPI_Allreduce(&sum, &sumout, 1, MPI_DOUBLE_PRECISION, MPI_MAX,
                      comm_i[d]);
    else
        sumout = sum;
    return sumout;
...
double Parallel::SumAll(const int d, const double sum) {
    ...
    if(npi[d] > 1)
        ...
        MPI_Allreduce(&sum, &sumout, 1, MPI_DOUBLE_PRECISION, MPI_SUM,
                      comm_i[d]);
    else
        sumout = sum;
    return sumout;
...
void Parallel::dreceive(const int d, const int tag, const int procfrom,
    ...
    MPI_Status status;
    if(npi[d] > 1)
        MPI_Recv(sum, n, MPI_DOUBLE_PRECISION, procfrom, tag, comm_i[d],
                 &status);
}
...
void Parallel::adreceive(const int d, const int tag, const int procfrom,
    ...
    if(npi[d] > 1)
        MPI_Irecv(sum, n, MPI_DOUBLE_PRECISION, procfrom, tag, comm_i[d],
                  &request[d][reqcnt[d]++]);
}
...
void Parallel::adsend(const int d, const int tag, const int procto, const int n,
                      double* sum) {
    if(npi[d] > 1)
        MPI_Isend(sum, n, MPI_DOUBLE_PRECISION, procto, tag, comm_i[d],
                  &request[d][reqcnt[d]++]);
}

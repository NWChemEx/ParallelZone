...
ProcGroup::ProcGroup( const ProcGroup * parent
		      ,       MPI_Comm         comm
		      ,       int              rank
		      ,       int              nRanks
		      ,       int              threads
...
void ProcGroup::setGlobalComm(int num_comms) const
{
  ...
  for (int i = curr_size; i < num_comms; i++) {
    if (ParallelZone::MPI::Comm_dup(m_comm, &m_global_comms[i]) != MPI_SUCCESS) {
      std::cerr << "Rank: " << m_rank << " - MPI Error in ParallelZone::MPI::Comm_dup\n";
      Parallel::exitAll(1);
    }
  }
}

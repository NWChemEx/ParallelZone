...
ProcGroup::ProcGroup(const ProcGroup* parent, MPI_Comm comm, int rank,
  ...
  m_threads(threads) {
    ...
    if(proc_name_map.size() > 1) {
        ...
        MPI::Comm_split(m_comm, m_all_proc_indexs[m_rank], m_rank,
                        &m_node_comm);
...
void ProcGroup::setGlobalComm(int num_comms) const {
    if(m_threads <= 1) { return; }

    int curr_size = m_global_comms.size();
    if(num_comms <= curr_size) { return; }

    m_global_comms.resize(num_comms);
    for(int i = curr_size; i < num_comms; i++) {
        if(ParallelZone::MPI::Comm_dup(m_comm, &m_global_comms[i]) !=
           MPI_SUCCESS) {
            std::cerr << "Rank: " << m_rank
                      << " - MPI Error in ParallelZone::MPI::Comm_dup\n";
            Parallel::exitAll(1);
        }
    }
}
...
std::string ProcGroup::myNodeName() const {
    return m_all_proc_names[m_all_proc_indexs[m_rank]];
}

// For any rank get the node index.
int ProcGroup::getNodeIndexFromRank(unsigned int rank) const {
...
std::string ProcGroup::getNodeNameFromRank(unsigned int rank) const {
    // Make sure the rank is valid.
    if(0 <= rank && rank < m_all_proc_indexs.size())
...
std::string ProcGroup::getNodeName(unsigned int node) const {
